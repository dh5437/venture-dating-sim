...
const init = async() => {
    ...
    const items = ['러브 레터', '꽃다발', '향수', '참이슬', '코트', '투명 뿔테 안경', '트레이닝 바지', '현금', '비트코인', '이더리움', '통장', '따뜻한 스웨터', '목도리', '현란한 카톡멘트'];
    ...
    while(maps.length<75){
        const mapCoordinate = remainingCoordinates.splice(Math.floor(Math.random()* remainingCoordinates.length), 1)[0];
