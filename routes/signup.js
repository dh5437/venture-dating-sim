...
router.post('/',
    ...
    async(req, res) => {
      ...
      try{
        ...
        user = new User({username : username, password : encryptedPassword, name : name, level : 1, maxhp : 100, hp : 100, exp : 0, str : 10, map : map});
        await user.save();
      } catch(err){
      ...
      for(const _item of items){
        const item = new Item({name : _item.name, quantity : 0, hp : _item.hp, exp : _item.exp, str : _item.str, user, isActive : true});
        await item.save();
      }
      res.send(200);
})
